...
public class Main {
    ...
    static int bfs(){
        ...
        while(!q.isEmpty()){
            ...
            if(cur.r==r-1 && cur.c==c-1) min = Math.min(cur.t, min);
            for(int i=0;i<4;i++){
                int rr = cur.r+dx[i];
                int cc = cur.c+dy[i];
                if(rr>=r|| cc>=c|| rr<0|| cc<0|| visited[rr][cc][cur.k]) continue;
                if(rr==r-1 && cc==c-1) min = Math.min(cur.t+1, min);
                if(miro[rr][cc]==0){
                    visited[rr][cc][cur.k] = true;
                    q.add(new Point(rr,cc,cur.k, cur.t+1));
                }else{
                    if(cur.k==0){//이 경우에만 최소 한번만 갈수있음
